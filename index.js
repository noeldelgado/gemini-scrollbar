...
(function () {
  ...
  function GeminiScrollbar(config) {
    ...
    SCROLLBAR_WIDTH = this.showBar ? getScrollbarWidth() || 17 : 0;
  ...
  GeminiScrollbar.prototype.create = function create() {
    ...
    if (this.createElements === true) {
      ...
      this._viewElement = this.element.querySelector("." + CLASSNAMES.view);
      this._scrollbarVerticalElement = this.element.querySelector(
        "." + CLASSNAMES.verticalScrollbar.split(" ").join(".")
      );
      this._thumbVerticalElement = this._scrollbarVerticalElement.querySelector(
        "." + CLASSNAMES.thumb
      );
      this._scrollbarHorizontalElement = this.element.querySelector(
        "." + CLASSNAMES.horizontalScrollbar.split(" ").join(".")
      );
      this._thumbHorizontalElement = this._scrollbarHorizontalElement.querySelector(
        "." + CLASSNAMES.thumb
      );
    }
    ...
    addClass(
      ...
      CLASSNAMES.verticalScrollbar.split(/\s/)
    );
    addClass(
      this._scrollbarHorizontalElement,
      CLASSNAMES.horizontalScrollbar.split(/\s/)
    );
  ...
  GeminiScrollbar.prototype.update = function update() {
    ...
    this._viewElement.style.width =
      (this.element.offsetWidth + SCROLLBAR_WIDTH).toString() + "px";
    this._viewElement.style.height =
      (this.element.offsetHeight + SCROLLBAR_WIDTH).toString() + "px";
    ...
    this._scrollTopMax =
      this._viewElement.scrollHeight - this._viewElement.clientHeight;
    this._scrollLeftMax =
      this._viewElement.scrollWidth - this._viewElement.clientWidth;
    ...
    this._trackTopMax =
      this._scrollbarVerticalElement.clientHeight - this._thumbSizeY;
    this._trackLeftMax =
      this._scrollbarHorizontalElement.clientWidth - this._thumbSizeX;
  ...
  GeminiScrollbar.prototype._bindEvents = function _bindEvents() {
    ...
    this._cache.events.clickVerticalTrackHandler = this._clickVerticalTrackHandler.bind(
      this
    );
    this._cache.events.clickHorizontalTrackHandler = this._clickHorizontalTrackHandler.bind(
      this
    );
    this._cache.events.clickVerticalThumbHandler = this._clickVerticalThumbHandler.bind(
      this
    );
    this._cache.events.clickHorizontalThumbHandler = this._clickHorizontalThumbHandler.bind(
      this
    );
    this._cache.events.mouseUpDocumentHandler = this._mouseUpDocumentHandler.bind(
      this
    );
    this._cache.events.mouseMoveDocumentHandler = this._mouseMoveDocumentHandler.bind(
      this
    );
  ...
  GeminiScrollbar.prototype._scrollHandler = function _scrollHandler() {
    var x =
      (this._viewElement.scrollLeft * this._trackLeftMax) /
        this._scrollLeftMax || 0;
    var y =
      (this._viewElement.scrollTop * this._trackTopMax) / this._scrollTopMax ||
      0;
  ...
  GeminiScrollbar.prototype._clickVerticalTrackHandler = function _clickVerticalTrackHandler(
    e
  ) {
    if (e.target !== e.currentTarget) {
      return;
    }
    var offset = e.offsetY - this._naturalThumbSizeY * 0.5,
      thumbPositionPercentage =
        (offset * 100) / this._scrollbarVerticalElement.clientHeight;

    this._viewElement.scrollTop =
      (thumbPositionPercentage * this._viewElement.scrollHeight) / 100;
  };

  GeminiScrollbar.prototype._clickHorizontalTrackHandler = function _clickHorizontalTrackHandler(
    e
  ) {
    if (e.target !== e.currentTarget) {
      return;
    }
    var offset = e.offsetX - this._naturalThumbSizeX * 0.5,
      thumbPositionPercentage =
        (offset * 100) / this._scrollbarHorizontalElement.clientWidth;

    this._viewElement.scrollLeft =
      (thumbPositionPercentage * this._viewElement.scrollWidth) / 100;
  };

  GeminiScrollbar.prototype._clickVerticalThumbHandler = function _clickVerticalThumbHandler(
    e
  ) {
    this._startDrag(e);
  ...
  GeminiScrollbar.prototype._clickHorizontalThumbHandler = function _clickHorizontalThumbHandler(
    e
  ) {
    this._startDrag(e);
  ...
  GeminiScrollbar.prototype._mouseMoveDocumentHandler = function _mouseMoveDocumentHandler(
    e
  ) {
    ...
    if (this._prevPageY) {
      offset =
        e.clientY - this._scrollbarVerticalElement.getBoundingClientRect().top;
    ...
    if (this._prevPageX) {
      offset =
        e.clientX -
        this._scrollbarHorizontalElement.getBoundingClientRect().left;
